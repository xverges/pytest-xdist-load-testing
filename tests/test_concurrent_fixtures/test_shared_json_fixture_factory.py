...
def test_creates_shared_json(pytester):
    ...
    pytester.makepyfile("""
        import pytest
        from pytest_load_testing import SharedJson
...
def test_on_last_worker_callback(pytester):
    ...
    assert marker_file.exists(), "on_last_worker callback was not called - marker file not found"
    content = marker_file.read_text()
    assert "callback_executed" in content, f"Unexpected marker content: {content}"
    assert "count=5" in content, f"Callback did not receive correct shared data: {content}"
